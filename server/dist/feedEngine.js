import { discoverLiveMarkets, getMarketState, checkMarketResolutionOnChain, } from './chain.js';
import { CONTRACT_ADDRESSES } from './config.js';
// In-memory store for user bets and resolved story notifications
export const userBetsStore = new Map();
export function recordUserBet(bet) {
...
export function getUserBets(walletAddress) {
...
function generateSparkline(currentProb) {
    const points = [];
...
function formatNewsHeadline(asset, intervalMinutes, currentProb) {
 ...
 */
export async function generateFeed(walletAddress, page = 1, limit = 8) {
    ...
    if (cachedBaseFeed.length > 0 && nowMs - lastFeedGenerationTime < FEED_CACHE_TTL_MS) {
        ...
        const marketCards = await Promise.all(combined.map(async (m, idx) => {
    ...
    if (returnMarketCards.length < limit) {
        ...
        const extraAssets = [
            { asset: 'BTC', cat: 'CRYPTO', headline: 'Bitcoin 15M: Rapid Scalp Window Active' },
            { asset: 'ETH', cat: 'DEFI', headline: 'Ethereum 30M: Layer 1 Gas Rush Prediction' },
            { asset: 'SOMNIA', cat: 'SOMNIA', headline: 'Somnia 5M: Ultra-fast Sub-second Settlement Pool' },
            { asset: 'SOL', cat: 'CRYPTO', headline: 'Solana 15M: High-frequency Up/Down Odds' },
            { asset: 'AI_AGENT', cat: 'TECH', headline: 'Autonomous AI: Trading Volume Surge Test' },
        ];
