...
import { getMarketState, placeBetOrder, redeemWinningOutcome, checkWalletCanBet, checkMarketResolutionOnChain, publicClient, } from './chain.js';
import { generateFeed, recordUserBet, getUserBets, userBetsStore, } from './feedEngine.js';
const app = express();
app.use(cors());
...
app.post('/api/bet', async (req, res) => {
    try {
        const { walletAddress, marketId, pool, asset, headline, direction, // 'UP' | 'DOWN'
        amountUsdc, privateKey, expiryTimestamp, } = req.body;
...
app.get('/api/portfolio/:wallet', async (req, res) => {
    try {
        ...
        let resolvedCount = 0;
        let winsCount = 0;
        const positions = await Promise.all(bets.map(async (b) => {
